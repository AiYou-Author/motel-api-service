...
- 🎫 **套餐管理**: 后台可视化创建/编辑/停用套餐，支持多渠道（Claude Code、Codex、MiniMax 等）
- 💰 **按量付费**: 用户充值后按实际消耗扣费，支持自定义倍率定价
- 📋 **订单管理**: 用户下单 → 管理员审批 → 自动发放 API Key
- 💳 **支付配置**: 后台可配置收款二维码和支付说明
- 📊 **价格展示**: 套餐卡片自动显示按倍率计算的模型价格（每百万 token）

### 🎯 模型白名单（新功能）

- 🔐 **按账号过滤**: `/v1/models` 接口根据 API Key 绑定的账号返回对应模型列表
- 👥 **分组支持**: 支持账号分组，API Key 绑定分组后自动继承分组内账号的 `supportedModels`
- 🏷️ **自定义模型**: 支持 MiniMax 等第三方模型，自动合并到模型列表
- ✨ **Claude Code 兼容**: 用户使用 `/model` 切换时只能看到被授权的模型
