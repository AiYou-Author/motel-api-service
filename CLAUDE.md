...
src/
├── routes/              # HTTP 路由
│   ├── api.js           # Claude API 主路由（含 Anthropic vendor 分流）
│   ├── admin/           # 管理后台路由（24个子文件）
...
│   ├── store.js, referral.js       # 商店和推荐路由
├── middleware/           # auth.js(认证/权限/限流), browserFallback.js, debugInterceptor.js
├── handlers/             # geminiHandlers.js
├── services/             # 业务服务（按域组织）
│   ├── relay/                 # 各平台转发服务（9个，关键服务已分解）
│   ├── account/               # 各平台账户管理（11个，关键服务已分解）
│   ├── scheduler/             # 统一调度器（4个，claude已分解）
│   ├── balanceProviders/      # 余额查询提供者（6个）
│   ├── auth/                  # apiKeyService, ldapService, userService 等
│   ├── billing/               # pricingService, costInitService, costRankService 等
│   ├── notification/          # webhookService, webhookConfigService
│   ├── transform/             # openaiToClaude, anthropicGeminiBridge 等格式转换
│   ├── routing/               # modelService, requestDetailService 等路由配置
│   ├── referral/              # 推荐服务
│   ├── store/                 # 商店服务
│   └── ...                    # 其余跨域服务
├── models/redis.js       # Redis 数据模型
├── utils/                # 工具函数（按域组织到9个子目录）
│   ├── logging/          # logger, tokenRefreshLogger 等
│   ├── http/             # proxyHelper, streamHelper 等
│   ├── security/         # tokenMask, errorSanitizer 等
│   ├── cache/            # lruCache, cacheMonitor 等
│   ├── request/          # costCalculator, sessionHelper 等
│   ├── upstream/         # upstreamErrorHelper, tempUnavailablePolicy 等
│   ├── diagnostics/      # anthropicRequestDump 等
│   ├── oauth/            # oauthHelper, antigravityModel 等
│   └── common/           # commonHelper, dateHelper, modelHelper 等
├── lib/                  # 纯工具函数（零项目依赖）
│   ├── crypto.js, formatters.js, validators.js, constants.js
├── bootstrap/            # 应用启动初始化
│   ├── redis.js, services.js, admin.js, index.js
...
- **Anthropic 厂商路由**: 通过 URL 前缀强制分流到特定 OAuth 账户类型：
  - `/api` → Claude 账号池（默认）
  - `/antigravity/api` → Antigravity OAuth（`req._anthropicVendor = 'antigravity'`）
  - `/gemini-cli/api` → Gemini CLI OAuth（`req._anthropicVendor = 'gemini-cli'`）
  - `/claude` → 与 `/api` 功能相同（别名路由）
...
- 强制花括号：`if`/`else`/`for`/`while` 必须使用 `{}`（`curly: ['error', 'all']`）
- 空行规则：`return` 前必须空行，`const`/`let`/`var` 后必须空行，连续声明之间不空行
- 最多一个连续空行，文件末尾不允许空行
...
注意：`npm run dev` (nodemon) 在每次文件变更重启时会自动先运行 lint，lint 失败则重启被跳过。
...
```bash
npm run service:start           # 启动服务（前台）
npm run service:start:d         # 启动服务（守护进程）
...
GitHub Actions (`.github/workflows/ci.yml`) 在 PR 和 push 到 `develop`/`main` 时触发：

| 阶段 | 内容 |
|------|------|
| **lint** | 后端 + 前端 ESLint + Prettier 检查 |
| **test-backend** | Jest 测试（依赖 lint 通过） |
| **test-frontend** | 前端测试（依赖 lint 通过） |
| **build** | 构建前端 SPA（依赖测试通过） |
| **docker-build** | Docker 镜像构建（仅 main 分支） |

提交 PR 前确保：`npm run lint` 和 `npm test` 都通过。
